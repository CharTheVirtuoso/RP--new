...
if transcript.status == aai.TranscriptStatus.completed:
    ...
    # Print the auto chapters
    if transcript.chapters:
        chapters = []
        for idx, chapter in enumerate(transcript.chapters):
            chapters.append((idx + 1, chapter.summary, format_time(chapter.start), format_time(chapter.end)))
        print("\nAuto Chapters:")
        for chap in chapters:
            print(f"Chapter {chap[0]}: {chap[1]} (Starts at {chap[2]}, Ends at {chap[3]})")
        ...
        # Save the HTML content to a file
        with open('video_chapters_test2.html', 'w') as file:
            file.write(html_output)
        
        print("HTML file 'video_chapters.html' has been generated.")
    else:
        print("No auto chapters detected.")
